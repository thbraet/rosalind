import pytest
from rosalind.string_algorithms import find_common_substring  # Replace 'your_module' with the actual module name

@pytest.mark.parametrize(
    "s1, s2, expected_result",
    [
        ("hello", "yellow", ['llo', 'ell', 'll', 'lo', 'o', 'ello', 'l', 'el', 'e']),  # Common substrings: ['e', 'ell', 'o', 'l', 'lo']
        ("abc", "def", []),                                # No common substrings
        ("abc", "abc", ["abc", "a", "b", "c", "ab", "bc"]), # Full overlap
        ("abcdef", "bcd", ["b", "bc", "bcd", "c", "cd", "d"]),  # Partial overlap
        ("short", "shorter", ["s", "sh", "sho", "shor", "short", "h", "ho", "hor", "hort", "o", "or", "ort", "r", "rt", "t"]),
...
def test_find_common_substring(s1, s2, expected_result):
    result = find_common_substring(s1, s2)
