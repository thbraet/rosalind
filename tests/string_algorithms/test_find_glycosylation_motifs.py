...
def test_valid_id_with_motifs():
    ...
    assert 85 in result, "Expected motif at position 85."
...
def test_valid_id_without_motifs():
    ...
    result = find_glycosylation_motifs(
        "A2Z669"
    )  # Replace with a real ID with no motifs
...
@pytest.mark.parametrize(
    ...
    [
        (
            "P10643_CO7_HUMAN",
            [202, 754],
        ),  # Known ID with motifs at positions 202 and 754
        ("Q3Z2Z2", [49]),  # Known ID with a motif at position 49
        (
            "P02725_GLP_PIG",
            [16, 19, 39],
        ),  # Known ID with motifs at positions 16, 19, and 39
        (
            "P07359_GPBA_HUMAN",
            [37, 175, 362, 398],
        ),  # Known ID with motifs at multiple positions
        (
            "P07585_PGS2_HUMAN",
            [211, 262, 303],
        ),  # Known ID with motifs at positions 211, 262, and 303
        (
            "P01880_DTC_HUMAN",
            [225, 316, 367],
        ),  # Known ID with motifs at positions 225, 316, and 367
        (
            "P72173",
            [87, 284, 383],
        ),  # Known ID with motifs at positions 87, 284, and 383
        (
            "Q90304_C166_CARAU",
            [92, 171, 350, 441, 465],
        ),  # Known ID with multiple motifs
    ],
)
def test_find_glycosylation_motifs(uniprot_id, expected_result):
    ...
    ), f"Expected {expected_result} for '{uniprot_id}', but got {result}."
